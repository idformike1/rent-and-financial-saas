...
export default function MassIngestionClient() {
  ...
  return (
    <div className="space-y-6 pb-10 max-w-7xl mx-auto animate-in fade-in duration-700">
      ...
      <div className={data.length > 0 ? "flex flex-col gap-12" : "grid grid-cols-1 lg:grid-cols-2 gap-6 items-start"}>

        {/* DROP ZONE */}
        <div
          onDragOver={handleDragOver}
          onDrop={handleDrop}
          className={`relative group glass-panel border-2 border-dashed transition-all duration-300 rounded-[8px] p-6 flex flex-col items-center justify-center text-center cursor-pointer ${
            file
        ...
        >
          {file ? (
            <div className="space-y-6 animate-in zoom-in-90 duration-500">
              <div className="w-20 h-20 bg-[var(--primary)] rounded-full flex items-center justify-center mx-auto shadow-none">
                <CheckCircle2 className="w-10 h-10 text-foreground" />
              </div>
              <div>
                <h3 className="text-2xl text-[var(--foreground)] ">{file.name}</h3>
                <p className="text-[var(--primary)] font-bold  text-xs mt-2">
        ...
        <div className="glass-panel rounded-[8px] border border-[var(--border)] overflow-hidden flex flex-col">
          <div className="bg-[var(--card)] px-8 py-5 border-b border-[var(--border)] flex items-center justify-between">
            ...
            {data.length > 0 && (
              <Badge variant="brand" className="bg-[var(--primary)]/10 text-[var(--primary)] border-transparent">
          ...
          <div className="p-0 flex-1 flex flex-col min-h-[320px]">
              ...
              <div className="max-h-[600px] overflow-y-auto w-full overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead className="sticky top-0 bg-[var(--card)]/90 border-b border-[var(--border)] z-10 shadow-sm">
                    <tr>
                      <th className="py-4 px-6 text-[10px]  text-[var(--muted)]">Date</th>
                      <th className="py-4 px-6 text-[10px]  text-[var(--muted)]">Payee</th>
                      <th className="py-4 px-6 text-[10px]  text-[var(--muted)]">Description</th>
                      <th className="py-4 px-6 text-[10px]  text-[var(--muted)]">Category</th>
                      <th className="py-4 px-6 text-[10px]  text-[var(--muted)] text-right">Amount</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[var(--border)]/50">
                    {data.slice(0, 100).map((row, idx) => (
                      <tr key={idx} className="hover:bg-[var(--primary)]/5 transition-colors group">
                        ...
                        <td className="py-4 px-6"><Badge variant="default" className="text-[9px] ">{row.Category || row.category || 'UNCLASSIFIED'}</Badge></td>
                        <td className="py-4 px-6 text-right text-sm font-finance tabular-nums text-[var(--primary)]">${row.Amount || row.amount || '0.00'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-6 bg-[var(--card)]/50 border-t border-[var(--border)] mt-auto flex flex-col gap-6">
              {errorStatus && (
                <div className="bg-rose-500/10 border border-rose-500/30 p-5 rounded-[8px] flex items-start gap-4 animate-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="mt-0.5">
                    ...
                    <p className="text-[var(--foreground)] text-xs font-bold  tracking-wider">{errorStatus}</p>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                className={`w-full py-5 rounded-[8px]  text-[11px] flex items-center justify-center gap-3 transition-all ${
                  data.length > 0 && !isProcessing && !errorStatus
                    ? 'bg-[var(--primary)] text-[var(--primary-foreground)] shadow-none hover:shadow-none hover:translate-y-[1px]'
