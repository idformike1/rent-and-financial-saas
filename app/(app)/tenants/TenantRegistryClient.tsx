...
export default function TenantRegistryClient({ tenants: initialTenants }: { tenants: any[] }) {
  ...
  return (
    <div className="space-y-12">
      ...
      <Card className="p-4 bg-slate-900 border-none rounded-3xl flex items-center gap-4 group transition-all duration-500 focus-within:ring-4 focus-within:ring-brand/10">
                    ...
                    <tr key={tenant.id} className="hover:bg-slate-50/80 dark:hover:bg-brand/5 transition-all group cursor-default">
                      ...
                      <td className="px-8 py-10">
                        <div className="flex flex-col gap-3">
                           <Badge variant={activeLeases.length > 0 ? 'success' : 'default'} className="px-4 py-1.5 text-[9px] rounded-full border-none shadow-premium-sm">
                             {activeLeases.length > 0 ? 'SIGNAL::ACTIVE' : 'SIGNAL::RESERVED'}
                           </Badge>
                           <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">{activeLeases.length} Active Nodes</span>
                        </div>
                      </td>
