...
export default function TenantProfileView({ tenant, activeLeases, charges, ledgerEntries }: TenantProfileViewProps) {
  ...
  return (
    <div className={cn("max-w-6xl mx-auto space-y-12 animate-in fade-in duration-500", tenant.isDeleted && "opacity-60")}>
      ...
      <div className="bg-background border border-border rounded-[3rem] overflow-hidden p-12 flex flex-col lg:flex-row justify-between items-center gap-12 relative">
         ...
         <div className="flex items-center gap-10 relative z-10">
            ...
            <div>
               ...
               <div className="flex flex-wrap gap-4 mt-6">
                  <span className="text-[10px] font-black text-muted-foreground flex items-center bg-muted/50 border border-border px-5 py-2 rounded-3xl uppercase tracking-widest"><Mail className="w-3.5 h-3.5 mr-3 text-brand" /> {tenant.email || 'N/A'}</span>
                  <span className="text-[10px] font-black text-muted-foreground flex items-center bg-muted/50 border border-border px-5 py-2 rounded-3xl uppercase tracking-widest"><Phone className="w-3.5 h-3.5 mr-3 text-brand" /> {tenant.phone || 'N/A'}</span>
                  <span className="text-[10px] font-black text-muted-foreground flex items-center bg-muted/50 border border-border px-5 py-2 rounded-3xl uppercase tracking-[0.2em]"><ShieldCheck className="w-3.5 h-3.5 mr-3 text-brand" /> ID::{tenant.nationalId || 'UNVERIFIED'}</span>
               </div>
            </div>
         </div>
         ...
         <div className="flex items-center gap-12 relative z-10">
            <div className="text-center space-y-2">
               <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Integrity Index</p>
               <div className={cn(
            ...
            <div className="h-20 w-px bg-muted hidden lg:block" />
            <div className="flex flex-col gap-6">
               {!tenant.isDeleted && (
                 <div className="flex flex-col gap-2">
                    ...
                    <button 
                      ...
                      className="bg-muted/50 text-foreground border border-border px-8 py-4 rounded-xl hover:bg-muted transition-all flex items-center justify-center gap-3 uppercase font-black tracking-widest text-[9px] font-mono h-14"
                    >
               ...
               {!tenant.isDeleted && (
                 <button 
                    ...
                    className="bg-transparent text-muted-foreground border border-border px-8 py-3 rounded-xl hover:text-foreground hover:border-border transition-all text-[9px] uppercase font-black tracking-[0.4em] font-mono h-12"
                 >
                    EDIT PROFILE
                 </button>
               )}
      ...
      <div className="space-y-6">
         <div className="flex justify-between items-end">
            <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] flex items-center gap-3 italic">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
        ...
        <div className="md:col-span-4 space-y-8">
           <div className="bg-card border border-border rounded-[3rem] p-10 relative overflow-hidden group">
              ...
              <p className="text-muted-foreground font-black uppercase tracking-[0.3em] text-[10px] mb-6">Aggregate Liability</p>
              ...
              <div className="mt-12 pt-8 border-t border-border flex justify-between items-center">
                 <span className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] italic">System Status</span>
                 <span className={cn(
           ...
           <div className="bg-background border border-border rounded-[3rem] p-10">
              <div className="flex justify-between items-center mb-10">
                 <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] flex items-center italic">
                    <Layers className="w-4 h-4 mr-4 text-brand" /> Active Assets
                 </h4>
                 {!tenant.isDeleted && (
                    <button onClick={() => setIsAddLeaseModalOpen(true)} className="bg-muted/50 text-foreground p-3 rounded-3xl hover:bg-muted transition-all border border-border">
              ...
              <div className="space-y-6">
                 {activeLeases.length === 0 ? (
                    <div className="py-12 text-center bg-card/[0.02] rounded-[2rem] border border-dashed border-border">
                       <p className="text-[10px] font-black text-muted-foreground uppercase italic tracking-widest leading-relaxed px-10">NO OPERATIONAL TENURE DETECTED IN CURRENT REALM</p>
                    </div>
                 ) : (
                    activeLeases.map(lease => (
                       <div key={lease.id} className="p-8 bg-muted/50 rounded-[2rem] border border-border relative group hover:border-brand/40 transition-all">
                          <div className="flex justify-between items-start mb-6">
                             ...
                             {!tenant.isDeleted && (
                                <button 
                                   ...
                                   className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-rose-500 transition-all active:scale-90"
                                >
                          ...
                          <div className="flex justify-between items-end">
                             <p className="text-2xl font-black text-foreground tracking-tighter font-finance tabular-nums italic">${lease.rentAmount.toLocaleString()}<span className="text-[10px] text-muted-foreground lowercase ml-1">/fisc</span></p>
                             <div className="text-right">
                                <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">Expiry_Window</p>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-card rounded-[3.5rem] w-full max-w-xl overflow-hidden border border-border relative">
              <button onClick={() => setIsEditModalOpen(false)} className="absolute top-12 right-12 text-muted-foreground hover:text-foreground transition-colors z-10"><XCircle className="w-10 h-10" /></button>
              <div className="p-12 border-b border-border bg-card/[0.02]">
                  ...
                  <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.4em] mt-6">Master Registry Forensic Mutation</p>
              </div>
              <form onSubmit={handleSubmit(onEditTenant)} className="p-16 space-y-10">
                 <div className="space-y-8">
                   <div>
                     <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Legal Aggregate Name</label>
                     <input {...register('name')} className="w-full bg-background border border-border rounded-[2rem] p-8 text-2xl font-black italic outline-none focus:border-brand transition-all text-foreground font-mono" />
                   </div>
                   <div className="grid grid-cols-2 gap-8">
                     <div>
                       <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Electronic Mail</label>
                       <input {...register('email')} className="w-full bg-background border border-border rounded-3xl p-6 text-sm font-bold outline-none focus:border-brand transition-all text-foreground" />
                     </div>
                     <div>
                       <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Telephonic Line</label>
                       <input {...register('phone')} className="w-full bg-background border border-border rounded-3xl p-6 text-sm font-bold outline-none focus:border-brand transition-all text-foreground" />
                     </div>
                   </div>
                   <div>
                     <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Structural Verification ID (SSN/Gov)</label>
                     <input {...register('nationalId')} className="w-full bg-background border border-border rounded-3xl p-8 text-xl font-black outline-none focus:border-brand transition-all text-foreground uppercase tracking-widest font-mono" />
                   </div>
                 </div>
      ...
      {isAddLeaseModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-card rounded-[3.5rem] w-full max-w-xl overflow-hidden border border-border relative">
              <button onClick={() => setIsAddLeaseModalOpen(false)} className="absolute top-12 right-12 text-muted-foreground hover:text-foreground transition-colors z-10"><XCircle className="w-10 h-10" /></button>
              ...
              <form onSubmit={handleLeaseSubmit(onAddLease)} className="p-16 space-y-10">
                 <div>
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Target Deployment Asset</label>
                    <select {...regLease('unitId')} className="w-full bg-background border border-border rounded-[2rem] p-8 text-xl font-black outline-none focus:border-brand transition-all appearance-none cursor-pointer text-foreground font-mono">
                 ...
                 <div className="grid grid-cols-2 gap-8">
                    <div>
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Monthly Fisc ($)</label>
                      <input type="number" {...regLease('rentAmount', {valueAsNumber: true})} className="w-full bg-background border border-border rounded-3xl p-6 text-sm font-bold text-foreground outline-none focus:border-brand transition-all" />
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] block mb-4 ml-6">Collateral ($)</label>
                      <input type="number" {...regLease('depositAmount', {valueAsNumber: true})} className="w-full bg-background border border-border rounded-3xl p-6 text-sm font-bold text-foreground outline-none focus:border-brand transition-all" />
                    </div>
                 </div>
                 <button disabled={isSubmitting} className="w-full h-24 bg-brand text-foreground font-black rounded-3xl shadow-[0_30px_60px_rgba(var(--brand-rgb),0.2)] hover:scale-[1.02] active:scale-98 disabled:opacity-50 transition-all uppercase tracking-[0.3em] text-[12px]">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-card rounded-[4rem] w-full max-w-lg overflow-hidden border border-rose-500/20 relative">
               <div className="p-16 text-center">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <button onClick={() => setIsDeleteModalOpen(false)} className="w-full text-muted-foreground font-black uppercase tracking-[0.5em] text-[8px] py-10 hover:text-foreground transition-colors">
                      [Abort_Protocol]
                    </button>
                  </div>
               </div>
      ...
      {isMoveOutModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-card rounded-[4rem] w-full max-w-lg overflow-hidden border border-border relative">
               <div className="p-16 text-center">
                  ...
                  <div className="flex flex-col gap-4">
                    <button onClick={onMoveOut} disabled={isSubmitting} className="w-full h-24 bg-card text-foreground font-black rounded-[2.5rem] shadow-[0_30px_60px_rgba(255,255,255,0.1)] hover:bg-muted transition-all active:scale-95 uppercase tracking-[0.4em] text-[11px]">
                      Execute Decommission
                    </button>
                    <button onClick={() => setIsMoveOutModalOpen(null)} className="w-full text-muted-foreground font-black uppercase tracking-[0.5em] text-[8px] py-8 hover:text-foreground transition-colors">
                      Maintain Assignment
                    </button>
                  </div>
               </div>
