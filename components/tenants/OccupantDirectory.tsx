...
export default function OccupantDirectory({ initialTenants }: { initialTenants: Tenant[] }) {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* CORE REGISTRY TABLE */}
      <Card className="p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem] bg-white dark:bg-slate-900">
        {filteredTenants.length === 0 ? (
          <div className="p-32 text-center space-y-6">
            <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto transition-transform hover:scale-110 border border-slate-100 dark:border-slate-800">
               <User className="w-10 h-10 text-slate-200 dark:text-slate-700" />
            </div>
            <div>
               <p className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">Identity Void</p>
                    ...
                    <tr key={tenant.id} className="hover:bg-slate-50/50 dark:hover:bg-brand/5 transition-all group">
                      <td className="px-8 py-8">
                        <div className="flex items-center gap-5">
                          <div className="h-14 w-14 rounded-2xl bg-slate-900 text-white flex items-center justify-center font-black text-xl italic shadow-xl group-hover:rotate-6 transition-all border border-white/10">
                      ...
                      <td className="px-8 py-8">
                        <Badge className={`px-4 py-1.5 text-[9px] rounded-xl border-none font-black tracking-widest ${activeLeases.length > 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-100 text-slate-400'}`}>
                          {activeLeases.length > 0 ? 'STATUS::ACTIVE' : 'STATUS::INACTIVE'}
                        </Badge>
                      </td>
                      <td className="px-8 py-8 text-right">
                        <Link href={`/tenants/${tenant.id}`}>
                           <button className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 flex items-center justify-center group-hover:bg-brand group-hover:text-white transition-all shadow-sm">
