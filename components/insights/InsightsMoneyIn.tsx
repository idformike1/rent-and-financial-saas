...
export default function InsightsMoneyIn({ data, entries }: { data: any[], entries: any[] }) {
  ...
  const tableData = React.useMemo(() => {
    ...
    entries.forEach(e => {
      // Filter for Money In (usually Income)
      if (e.account?.category !== "INCOME") return;
      
      let key = "Other";
      if (subTab === 'Source') key = e.account?.name || "Unknown";
      else if (subTab === 'Category') key = e.account?.type || "Revenue";
      else if (subTab === 'GL Code') key = `400${e.account?.id?.slice(-1) || '1'}`;
      
      map[key] = (map[key] || 0) + Math.abs(e.amount);
    });
    
    const totalIn = Object.values(map).reduce((a, b) => a + b, 0);
    return Object.entries(map)
      .map(([name, amount]) => ({
        name,
        amount,
        percent: Number((totalIn > 0 ? (amount / totalIn) * 100 : 0).toFixed(1))
      }))
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-12 mb-0 border-b border-[#2D2E39]/50 pb-8 px-4">
        <div className="flex flex-col gap-1">
          <p className="text-[15px] leading-[24px] font-normal text-[#F4F5F9] mb-1">Total money in</p>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left text-[15px] leading-[24px] text-[#F4F5F9] font-normal pb-2 w-1/3">Source</th>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="h-[6px] w-full max-w-[120px] bg-white/10 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${row.percent}%`, backgroundColor: '#3B82F6' }}
                        />
                      </div>
