...
export default function DrillDownDrawer({ categoryName, isOpen, onClose }: DrillDownDrawerProps) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-xl bg-white shadow-2xl z-[101] flex flex-col animate-in slide-in-from-right duration-300 border-l-8 border-black">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          {isLoading ? (
            ...
            <div className="space-y-4">
               {transactions.map((t) => (
                  <div key={t.id} className="p-6 border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-2xl bg-white hover:translate-y-0.5 hover:shadow-none transition-all flex items-center justify-between group">
                    ...
                    <div className="text-right">
                       <p className={`text-lg font-black italic tracking-tighter ${t.account?.category === 'EXPENSE' ? 'text-red-600' : 'text-green-600'}`}>
