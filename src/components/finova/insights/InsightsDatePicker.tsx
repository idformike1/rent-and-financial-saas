...
import { Calendar } from "@/src/components/finova/ui/calendar"
import { Field, FieldLabel } from "@/src/components/finova/ui/field"
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
  ...
  return (
    <div className={cn("w-fit", className)}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent 
          className="w-auto p-0 flex flex-row bg-popover border-white/10   rounded-[var(--radius-sm)] overflow-hidden" 
          align="start"
          sideOffset={8}
        >
          ...
          <div className="w-[140px] border-r border-white/5 p-2 flex flex-col gap-1 bg-white/[0.005]">
            ...
            {[
              ...
              <button
                ...
                className="text-left px-3 py-1.5 text-[12px] text-white/40 hover:text-white hover:bg-white/5 rounded-[var(--radius-sm)] transition-all font-medium"
              >
                {p.label}
              </button>
            ))}
          ...
          <Calendar
            ...
            classNames={{
              nav_button: "h-7 w-7 bg-white/5 hover:bg-white/10 text-white rounded-[var(--radius-sm)] transition-all",
              caption_label: "text-sm font-semibold tracking-clinical text-white",
              head_cell: "text-white/20 font-bold text-[11px] uppercase p-2 w-9",
              day: "h-9 w-9 text-sm font-medium hover:bg-white/5 rounded-[var(--radius-sm)] transition-all transition-colors",
            }}
          />
        </PopoverContent>
      </Popover>
